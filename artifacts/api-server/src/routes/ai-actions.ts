...
${mod.assignment}`;
  ...
  switch (action) {
    case "study-guide":
      return {
        system:
          "You are a college composition instructor preparing a concise study guide for a student. Output Markdown. Use these sections in order: '## Key concepts' (5–10 bulleted definitions), '## Core moves' (the main rhetorical or analytical moves in the reading), '## Common pitfalls' (3–5 mistakes students typically make on this topic), '## Self-check questions' (5 questions a student should be able to answer). Keep it tight — under 500 words. Do not write the assignment for the student.",
    ...
    case "tutorial":
      return {
        system:
          "You are a college composition instructor giving a step-by-step walkthrough that teaches the material in the reading. Output Markdown. Structure: a 1-paragraph plain-English intro, then 4–6 numbered steps, each with a heading, a 2–4 sentence explanation, and a short example. End with a short paragraph titled '## What to do next' that nudges the student toward the assignment WITHOUT writing any of it for them. Under 700 words.",
    ...
    case "podcast":
      return {
        system:
          "You are scripting a short audio explainer (a 2–3 minute 'podcast') that a student can listen to while commuting. Write a single-host, conversational script. NO speaker labels, NO stage directions, NO headings, NO Markdown — just the spoken words, in flowing paragraphs. Open with a friendly hook ('Welcome back to English Composition 101…'), explain the core ideas of the reading in plain language, weave in one concrete example, and close by inviting the student to think about the assignment prompt. Aim for roughly 350–450 words.",
    ...
    case "rewrite":
      return {
        system:
          "You are an editor rewriting a composition course reading for maximum clarity for a first-time undergraduate. Preserve every claim and example from the original — do not add new content or remove ideas. Use shorter sentences, simpler vocabulary, and clearer transitions. Output Markdown. Do not add commentary; output only the rewritten reading.",
        user: `Rewrite this reading for clarity. Keep all the content; just make it easier to follow.\n\n# Original reading\n${mod.reading || "(none)"}\n\n# Module context\n${mod.title}\nObjectives:\n${mod.objectives.map((o) => `- ${o}`).join("\n")}`,
        maxTokens: 2500,
      };
    case "read-draft": {
