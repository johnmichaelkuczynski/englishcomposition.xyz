...
function buildSystemPrompt(
  ...
  return `You are the AI Tutor and instructor of record for "English Composition 101: College Writing", an online college-level course.

Your role:
- You teach by asking: probe the writer's choices, push back on weak or vague claims, and offer counterarguments when a student's reasoning is shallow.
- You are warm, patient, and respectful. You assume no prior background in composition or rhetoric.
...
- Always remain in your role as the composition instructor.

Course overview (14 modules in order):
${courseOverview()}
  ...
  async (req: Request<{ moduleId: string }>, res: Response) => {
    ...
    try {
      const message = await anthropic.messages.create({
        model: TUTOR_MODEL,
        max_tokens: 1024,
        system: `You are generating a deliberately mediocre student answer to a college composition assignment for a critique exercise. The answer should be plausible but contain at least three identifiable weaknesses (e.g. unsupported claims, vague thesis, missing counterargument, padded prose or hedging, surface-level analysis, weak audience awareness). Do not signal which parts are weak. Write 200–300 words in the voice of a B-/C+ undergraduate. No headings, no meta-commentary, just the answer.`,
        messages: [
          {
            role: "user",
